...
const NamuWikiQuiz = () => {
  ...
  const openWikiWindow = useCallback((url: string, shouldStartTimer: boolean = false) => {
    // 기존 창 닫기
    ...
    <>
        ...
        <Layout>
          ...
          <GameContent>
            <SetupSection>
              ...
              <StartButton onClick={prepareGame} disabled={useCustomWords && (!customSource || !customDestination)}>
                🎲 문제 뽑기
              </StartButton>
      ...
      {isPlaying && currentPair && (
        <GamePlayingLayout>
          ...
          <BottomControlBar>
            <ControlGroup>
              <SmallButton onClick={reopenWikiWindow}>
                🔗 새 창
              </SmallButton>
          ...
          <GameContent>
            <ResultSection $result={lastResult}>
              <ResultBadge $result={lastResult}>
                {lastResult === 'success' ? '🎉 성공!' : lastResult === 'timeout' ? '⏰ 시간 초과!' : '😢 실패!'}
              </ResultBadge>
              
              <ResultDetails>
                <ResultRow>
                  <ResultLabel>경로</ResultLabel>
                  <ResultValue>{currentPair.source} → {currentPair.destination}</ResultValue>
                </ResultRow>
                <ResultRow>
